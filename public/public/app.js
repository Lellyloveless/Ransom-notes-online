...
socket.on("roundStarted",d=>{show("game");myVote=null;$("round").textContent="ROUND "+d.round;$("prompt").textContent=d.prompt;$("winner").classList.add("hidden");$("submissions").classList.add("hidden");$("status").textContent="";chosen=[];startTimer(d.deadline);renderTiles();renderSentence();renderScoreboard();});
socket.on("allSubmitted",()=>{$("status").textContent="Everyone has submitted — vote!";clearInterval(timer);});
socket.on("timeUp",()=>{$("status").textContent="Time's up! Your note was submitted automatically.";clearInterval(timer);});
