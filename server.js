...
function finishWriting(code, reason = "submitted") {
  ...
  // Auto-submit for all players (connected or not), with empty drafts if needed
  for (const [id, p] of Object.entries(r.players)) {
    if (!r.submissions[id]) {
      const draft = Array.isArray(p.draft) ? p.draft.slice(0, 100) : [];
      r.submissions[id] = {
        name: p.name,
        words: draft,
        auto: true,
...
io.on("connection", (s) => {
  s.on("createRoom", ({ name }, cb) => {
    ...
    s.join(code);
    s.data.room = code;
    cb({ ok: true, roomCode: code, playerId: s.id });
    broadcast(code);
  });
  ...
  s.on("joinRoom", ({ roomCode, name }, cb) => {
    ...
    if (Object.keys(r.players).length >= 8) {
      r.spectators[s.id] = { name: name.trim().slice(0, 24) };
      s.join(code);
      s.data.room = code;
      cb({ ok: true, spectator: true, roomCode: code, playerId: s.id });
      broadcast(code);
      return;
    }
  ...
  s.on("setRoundTime", ({ roomCode, minutes }, cb = () => {}) => {
    const r = rooms.get(roomCode);
    const n = Number(minutes);
    if (!r) return cb({ error: "Room not found." });
    if (s.id !== r.host) return cb({ error: "Only the host can choose the time." });
    if (!Number.isInteger(n) || n < 2 || n > 10)
      return cb({ error: "Choose between 2 and 10 minutes." });
    if (r.phase !== "lobby")
      return cb({ error: "Time can only be changed in the lobby." });
    r.roundMinutes = n;
    broadcast(roomCode);
    cb({ ok: true });
  });
  ...
  s.on("startRound", ({ roomCode }, cb = () => {}) => {
    const r = rooms.get(roomCode);
    if (!r) return cb({ error: "Room not found." });
    if (s.id !== r.host) return cb({ error: "Only the host can start." });

    const activeCount = activePlayerIds(r).length;
    if (activeCount < 3)
      return cb({ error: "At least 3 connected players are required." });

    r.round++;
    r.phase = "writing";
    r.prompt = prompts[Math.floor(Math.random() * prompts.length)];
    r.submissions = {};
    r.votes = {};
    r.deadline = Date.now() + r.roundMinutes * 60 * 1000;

    for (const [id, p] of Object.entries(r.players)) {
      p.tiles = makeTiles();
      p.draft = [];
      io.to(id).emit("yourTiles", p.tiles);
    }

    io.to(roomCode).emit("roundStarted", {
      prompt: r.prompt,
      round: r.round,
      minutes: r.roundMinutes,
      deadline: r.deadline,
    });

    broadcast(roomCode);

    r.timer = setTimeout(() => finishWriting(roomCode, "timeout"), r.roundMinutes * 60 * 1000);
    cb({ ok: true });
  });

  s.on("updateDraft", ({ roomCode, words: chosen }, cb = () => {}) => {
    const r = rooms.get(roomCode);
    const p = r?.players[s.id];
    if (!p || r.phase !== "writing")
      return cb({ error: "Draft not accepted." });
    if (
      !Array.isArray(chosen) ||
      chosen.length > 100 ||
      !chosen.every((w) => p.tiles.includes(w))
    )
      return cb({ error: "Use only your supplied words." });
    p.draft = chosen;
    cb({ ok: true });
  });
  ...
  s.on("submit", ({ roomCode, words: chosen }, cb = () => {}) => {
    ...
    const p = r?.players[s.id];
    if (!p || r.phase !== "writing")
      return cb({ error: "Submissions are closed." });
    ...
    const activeCount = activePlayerIds(r).length;
    if (Object.keys(r.submissions).length >= activeCount) {
      finishWriting(roomCode, "submitted");
    } else {
      broadcast(roomCode);
    }
  ...
  s.on("vote", ({ roomCode, target }, cb = () => {}) => {
    ...
    if (
      ...
      !r.players[s.id] ||
      s.id === target ||
      !r.submissions[target]
    )
      return cb({ error: "Vote not accepted." });

    if (r.votes[s.id]) return cb({ error: "You have already voted." });

    r.votes[s.id] = target;

    const activeCount = activePlayerIds(r).length;

    if (Object.keys(r.votes).length === activeCount) {
      const counts = {};
      Object.values(r.votes).forEach(
        (t) => (counts[t] = (counts[t] || 0) + 1)
      );
      const max = Math.max(...Object.values(counts));
      const winners = Object.keys(counts).filter((id) => counts[id] === max);
      winners.forEach((id) => r.players[id].score++);
      r.phase = "results";
      io.to(roomCode).emit("roundWinner", {
        names: winners.map((id) => r.players[id].name),
        scores: Object.fromEntries(
          winners.map((id) => [id, r.players[id].score])
        ),
      });
    }
...
s.on("requestState", (roomCode) => {
  ...
  s.on("disconnect", () => {
    ...
    if (r.players[s.id]) r.players[s.id].connected = false;
    delete r.spectators[s.id];

    if (r.host === s.id) {
      const next = Object.keys(r.players).find((id) => r.players[id].connected);
      if (next) r.host = next;
    }
